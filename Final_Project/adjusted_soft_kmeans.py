...
class SoftKMeansSplitMerge:
    def __init__(self, clusters_num=3, max_iter1=100, max_iter2 = 10, beta = 1.0, tol = None, random_state=None):
    ...
    def fit(self, X, init_type = 1, init_centroids = None):
        # 使用soft_kmeans获得最初的聚类结果
        soft_kmeans = skm.SoftKMeans(self.clusters_num, self.max_iter1, self.beta, self.tol, self.random_state)
        soft_kmeans.fit(X, init_type, init_centroids)
        self.labels = soft_kmeans.predict(X)
        self.centroids = soft_kmeans.return_centroids()
        # 尝试对聚类进行拆分合并去优化聚类的结果
        for k in range(0, self.max_iter2):
            ...
            soft_kmeans_temp = skm.SoftKMeans(clusters_num=2, max_iter=1, beta = self.beta, tol = self.tol, random_state = self.random_state)
            soft_kmeans_temp.fit(cluster_points, init_type= 4, init_centroids=np.array(farthest_points))
            labels_temp = soft_kmeans_temp.predict(cluster_points)
            self._update_labels(X, labels_temp, split_class, soft_kmeans_temp.return_centroids())
            # 更新类中心
